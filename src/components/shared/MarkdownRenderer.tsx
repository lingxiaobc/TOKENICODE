...
// Sanitize schema: GitHub defaults + className on all elements (needed for highlight.js and KaTeX)
const SANITIZE_SCHEMA = {
  ...
  attributes: {
    ...
    span: [...(defaultSchema.attributes?.span || []), 'style', 'aria-hidden'],
    div: [...(defaultSchema.attributes?.div || []), 'className', 'style'],
  },
  // Allow KaTeX math tags
  tagNames: [
    ...(defaultSchema.tagNames || []),
    'math', 'semantics', 'mrow', 'mi', 'mo', 'mn', 'msup', 'msub',
    'mfrac', 'mtext', 'annotation', 'msqrt', 'mroot', 'mtable', 'mtr', 'mtd',
  ],
};
...
async function loadRemarkPlugins(): Promise<RemarkPlugin[]> {
  ...
  if (!remarkPluginsPromise) {
    remarkPluginsPromise = import('remark-gfm')
      .then((mod) => {
        cachedRemarkPlugins = [mod.default, remarkMath];
        return cachedRemarkPlugins;
      })
      .catch((error) => {
